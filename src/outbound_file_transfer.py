...
from exceptions import InvalidAuthTypeError
...
def create_sftp_client(host, port):
    ...
    try:
        ...
        LOGGER.error('Error at %s', 'Could not establish sftp connection',
                     exc_info=sftp_error)
        bail_out(1)
...
class S3Ftp:
    def __init__(self):
        LOGGER.info("FTPIngestion Class Initializing")
        ...
        self.auth_status = get_auth_type(AUTH_METHOD)
        self.get_secret()
    ...
    def get_secrets(self):
        ...
        try:
            ...
            if error.response['Error']['Code'] == 'DecryptionFailureException':
                LOGGER.error('Error at %s', 'DecryptionFailureException',
                             exc_info=error)
            ...
            if error.response['Error']['Code'] == \
                ...
                LOGGER.error('Error at %s', 'InternalServiceErrorException',
                             exc_info=error)
            ...
            if error.response['Error']['Code'] == 'InvalidParameterException':
                LOGGER.error('Error at %s', 'InvalidParameterException',
                             exc_info=error)
                # print("ClientError: %s" % e)
                bail_out(1)
                raise error
            ...
            if error.response['Error']['Code'] == 'InvalidRequestException':
                LOGGER.error('Error at %s', 'InvalidRequestException',
                             exc_info=error)
                # print("ClientError: %s" % e)
                bail_out(1)

            if error.response['Error']['Code'] == 'ResourceNotFoundException':
                LOGGER.error('Error at %s', 'ResourceNotFoundException',
                             exc_info=error)
                # print("ClientError: %s" % e)
                bail_out(1)
        else:
